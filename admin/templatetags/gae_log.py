...
class AdminLogNode(template.Node):
    ...
    def render(self, context):
        if self.user is None:
            context[self.varname] = LogEntry.all().filter('content_type', 'user').fetch(int(self.limit))
        else:
            user_id = self.user
            if not user_id.isdigit():
                user_id = context[self.user].id
            context[self.varname] = LogEntry.all().filter('user_id', user_id).order('-action_time').fetch(int(self.limit))
        return ''
